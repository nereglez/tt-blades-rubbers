...
import { toESN, renderStars, typeTag, nivelColor } from '../utils.js';
  ...
  const html = items.map(item => {
    ...
    <div class="card ${inCompare ? 'in-compare' : ''}" data-id="${item.id}">
      ...
      <div class="card-footer">
        ...
        ${isRubber
          ? `<span class="card-hardness" style="font-size:0.72rem;color:#c084fc" title="Dureza declarada: ${item.hardness}${item.hardScale || ''}">${toESN(item.hardness, item.hardScale)}°ESN</span>`
